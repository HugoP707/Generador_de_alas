...
class Point:
	...
	def __init__(self, x, y, z, mesh_size):
		...
		self.tag = gmsh.model.occ.addPoint(
...
class Line:
	...
	def __init__(self, start_point, end_point):
		...
		# create the gmsh object and store the tag of the geometric object
		self.tag = gmsh.model.occ.addLine(
...
class Circle:
	...
	def __init__(self, xc, yc, zc, radius, mesh_size):
		...
		print(center.tag)
		# gmsh.model.occ.remove([(center.dim, center.tag)])
...
class AirfoilSpline:
	...
	def refinar_bordes(self, size, theta, ratioref=0.25):
		return size * (ratioref + ( (1 - ratioref) * (1 - np.cos(theta) ** 2 )) )
	...
	def gen_skin(self):
		...
		extra = int(CONST_EXTRA_UPPER*len(self.points))
		print("Extra en la upper: ", extra)
		self.upper_spline = Spline(
...
class AirfoilMultiLine:
	"""
	A class to represent and airfoil as a CurveLoop object formed with Multiple Lines
	...
	...
	def close_loop(self):
		...
		_ : int
			return the tag of the CurveLoop object
		"""
		self.close_loop_tag = CurveLoop(self.lines).tag
		return self.close_loop_tag
...
class PlaneSurface:
	...
	def __init__(self, geom_objects, preview_geom=False):
