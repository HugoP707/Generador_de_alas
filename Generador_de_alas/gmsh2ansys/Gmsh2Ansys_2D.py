...
def Gmsh2Ansys_2D(gmshFileName, ansysFileName):
   ...
   nodes_list = []
   elements_list = []
   # El codigo de matlab guarda la variable cuando sale del scope del for loop, como python no, guardarlo aqui
   zonde_id_last = None
   zone_ids = []
   zone_id_names = []
   ...
   while True:
      ...
      if xp == "$Nodes":
         ...
         for _ in range(no_nodes):
               ...
               if not line:
                  continue
               nodes_list.append(list(map(float, line.split())))
   ...
   zone_id = zone_ids[-1]
   elemF = elements[elements[:, 0] == zone_id, 0:]
   ...
   print(elemF[1])
   ax = 1
   print(np.sort(np.unique(elemF, axis=1)))
   print(np.sort(np.unique(elemF, axis=1)).shape)
   print(node.shape)

   nodeF = node[np.sort(np.unique(elemF, axis=0)), :]
   I_F = elemF.flatten()
